...
namespace rbp
{
	...
	MaxRectsBinPack::MaxRectsBinPack(int width, int height, bool canFlip)
	{
		Init(width, height, canFlip);
	}

	void MaxRectsBinPack::Init(int width, int height, bool canFlip)
	{
		binWidth = width;
		binHeight = height;
		binCanFlip = canFlip;
	...
	Rect MaxRectsBinPack::FindPositionForNewNodeBottomLeft(int width, int height, int& bestY, int& bestX) const
	{
		...
		for (size_t i = 0; i < freeRectangles.size(); ++i)
		{
			...
			if (binCanFlip && freeRectangles[i].width >= height && freeRectangles[i].height >= width)
			{
